...
async def upload_resume(
    ...
    try:
        ...
        ollama = get_ollama_client()
        agent = ResumeAdvisorAgent(ollama.get_model("reasoning"))
...
async def tailor_resume_endpoint(request: ResumeTailorRequest):
    ...
    try:
        ollama = get_ollama_client()
        agent = ResumeAdvisorAgent(ollama.get_model("fast"))
...
async def generate_resume_pdf(request: ResumeTailorRequest):
    ...
    try:
        ...
        await run_in_threadpool(generate_pdf, output_path, request.persona)
    ...
    try:
        ...
        ollama = get_ollama_client()
        llm = ollama.get_model("fast")  # Use fast model for quick parsing
...
Return ONLY the JSON object, no other text."""
        ...
        # Direct LLM call (much faster than LangGraph)
    ...
    async def generate():
        try:
            ...
            try:
                ...
                updater = ProfileUpdaterAgent(get_ollama_client().get_model("fast"))
...
async def agent_hunt_endpoint(request: JobHuntRequest):
    ...
    try:
        ...
        ollama = get_ollama_client()
        agent = JobHunterAgent(ollama.get_model("reasoning"))
